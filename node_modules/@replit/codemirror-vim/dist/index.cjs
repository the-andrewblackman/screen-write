...
function runHistoryCommand(cm, revert) {
    ...
    (revert ? commands.undo : commands.redo)(cm.cm6);
...
"Left|Right|Up|Down|Backspace|Delete".split("|").forEach(key => {
    keys[key] = (cm) => View.runScopeHandlers(cm.cm6, { key: key }, "editor");
});
class CodeMirror {
    constructor(cm6) {
        this.state = {};
    ...
    foldCode(pos) {
        ...
        let tmpRanges = state.EditorSelection.create([state.EditorSelection.range(index, index)], 0).ranges;
        view.state.selection.ranges = tmpRanges;
        language.foldCode(view);
    ...
    setSelections(p, primIndex) {
        ...
        var ranges = p.map(x => {
            ...
            if (head == anchor)
                return state.EditorSelection.cursor(head, 1);
            return state.EditorSelection.range(anchor, head);
        });
        this.cm6.dispatch({
            selection: state.EditorSelection.create(ranges, primIndex)
        });
    }
    ;
    ...
    getLineNumber(handle) {
        ...
        for (var i = 0; i < updates.length; i++) {
            offset = updates[i].changes.mapPos(offset, 1, state.MapMode.TrackAfter);
            if (offset == null)
                return null;
        }
    ...
    setValue(text) {
        ...
        return cm.dispatch({
            ...
            selection: state.EditorSelection.range(0, 0)
        });
    }
    ;
    focus() {
    ...
    findMatchingBracket(pos, _options) {
        ...
        var m = language.matchBrackets(state, offset + 1, -1);
        if (m && m.end) {
            return { to: posFromIndex(state.doc, m.end.from) };
        }
        m = language.matchBrackets(state, offset, 1);
        if (m && m.end) {
            return { to: posFromIndex(state.doc, m.end.from) };
        }
        return { to: undefined };
    ...
    ;
    indentMore() {
        commands.indentMore(this.cm6);
    }
    ;
    indentLess() {
        commands.indentLess(this.cm6);
    }
    ;
    execCommand(name) {
        if (name == "indentAuto")
    ...
    ;
    addOverlay({ query }) {
        let cm6Query = new search.SearchQuery({
            regexp: true,
            search: query.source,
        ...
        if (cm6Query.valid) {
            ...
            let effect = search.setSearchQuery.of(cm6Query);
    ...
    removeOverlay(overlay) {
        ...
        let effect = search.setSearchQuery.of(this.cm6Query);
        this.cm6.dispatch({ effects: effect });
    }
    ;
    getSearchCursor(query, pos) {
        ...
        function rCursor(doc, from = 0, to = doc.length) {
            return new search.RegExpCursor(doc, source, { ignoreCase: query.ignoreCase }, from, to);
        }
    ...
    findPosV(start, amount, unit, goalColumn) {
        ...
        let range = state.EditorSelection.cursor(startOffset, 1, undefined, goalColumn);
    ...
    scrollIntoView(pos, margin) {
        if (pos) {
            ...
            this.cm6.dispatch({
                effects: View.EditorView.scrollIntoView(offset)
            });
        }
        else {
    ...
    getOption(name) {
        switch (name) {
            ...
            case "indentWithTabs": return this.cm6.state.facet(language.indentUnit) == "\t"; // TODO
            case "indentUnit": return this.cm6.state.facet(language.indentUnit).length || 2;
    ...
    getTokenTypeAt(pos) {
        ...
        var offset = this.indexFromPos(pos);
        var tree = language.ensureSyntaxTree(this.cm6.state, offset);
    ...
    overWriteSelection(text) {
        ...
        var ranges = sel.ranges.map(x => {
            if (x.empty) {
                ...
                if (ch && !/\n/.test(ch))
                    return state.EditorSelection.range(x.from, x.to + 1);
            }
            return x;
        });
        this.cm6.dispatch({
            selection: state.EditorSelection.create(ranges, sel.mainIndex)
        });
    ...
    forEachSelection(command) {
        ...
        this.virtualSelection = state.EditorSelection.create(selection.ranges, selection.mainIndex);
        for (var i = 0; i < this.virtualSelection.ranges.length; i++) {
            var range = this.virtualSelection.ranges[i];
            if (!range)
                continue;
            this.cm6.dispatch({ selection: state.EditorSelection.create([range]) });
            command();
...
// --------------------------
CodeMirror.Pos = Pos;
CodeMirror.StringStream = language.StringStream;
CodeMirror.commands = {
    cursorCharLeft: function (cm) { commands.cursorCharLeft(cm.cm6); },
...
function findEnclosingTag(cm, pos) {
    ...
    var tree = language.ensureSyntaxTree(state, offset);
...
class Marker {
    ...
    update(change) {
        if (this.offset != null)
            this.offset = change.mapPos(this.offset, this.assoc, state.MapMode.TrackDel);
    }
}
...
const vimStyle = View.EditorView.baseTheme({
...
const vimPlugin = View.ViewPlugin.fromClass(class {
        ...
        if (update.transactions) {
            for (let tr of update.transactions)
                for (let effect of tr.effects) {
                    if (effect.is(search.setSearchQuery)) {
    ...
    highlight(query) {
        this.query = query;
        if (!query)
            return (this.decorations = View.Decoration.none);
        let { view } = this;
    ...
    provide: () => {
        return [
            View.EditorView.inputHandler.of((view, from, to, text) => {
                var _a, _b;
                var cm = getCM(view);
                if (!cm)
                    return false;
...
const matchMark = View.Decoration.mark({ class: "cm-searchMatch" });
...
function vim(options = {}) {
    return [
        ...
        vimPlugin,
        hideNativeSelection,
        options.status ? View.showPanel.of(statusPanel) : vimPanelState,
    ];
}
function getCM(view) {
    return view.cm || null;
